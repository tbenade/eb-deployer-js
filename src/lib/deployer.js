...
function configureStateMachine(config, services, options) {
  ...
  return new FSM(states).bind(FSM.EXIT, stateMachineTransitionHandler("exit")).bind(FSM.ENTER, stateMachineTransitionHandler("enter")).bind(
    FSM.CHANGE, stateMachineTransitionHandler("activate"));
}
