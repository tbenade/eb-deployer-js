    ...
    return {
        activate: function (fsm, data) {
            ...
            getEnvironments(config.ApplicationName).then(function (result) {
                    ...
                    var activeCname = helpers.calculateCnamePrefix(config.ApplicationName, args.environment, true), inactiveCname = helpers.calculateCnamePrefix(config.ApplicationName, args.environment, false), activeEnvironment = _.find(result.Environments, {CNAME: activeCname + '.elasticbeanstalk.com'}), inactiveEnvironment = _.find(result.Environments, {CNAME: inactiveCname + '.elasticbeanstalk.com'});
