...
l = require('../../../lib/logger.js'), helpers = require('../../../lib/helpers');

module.exports = function (config, services, args) {

    var eb = new services.AWS.ElasticBeanstalk(), region = config.Region;

    function getEnvironments(applicationName) {
        return Q.ninvoke(eb, "describeEnvironments", {
            ApplicationName: applicationName, IncludeDeleted: false
        });
    }

    return {
        activate: function (fsm, data) {
            getEnvironments(config.ApplicationName).then(function (result) {
                    l.info("Preparing environment to deploy version %s.", data.versionLabel);

                    var activeCname = helpers.calculateCnamePrefix(config.ApplicationName, args.environment, true), inactiveCname = helpers.calculateCnamePrefix(config.ApplicationName, args.environment, false), activeEnvironment = _.find(result.Environments, {CNAME: activeCname + '.elasticbeanstalk.com'}), inactiveEnvironment = _.find(result.Environments, {CNAME: inactiveCname + '.elasticbeanstalk.com'}), action = "next";
                    ...
                    if (activeEnvironment && inactiveEnvironment) {
                        ...
                        data.targetEnvironment = {
                            name: inactiveEnvironment.EnvironmentName, cname: inactiveCname, isActive: false
                        };
                        ...
                        var activeSuffix = helpers.getSuffixFromEnvironmentName(activeEnvironment.EnvironmentName), inactiveSuffix = activeSuffix == 'a' ? 'b' : 'a';
                        ...
                        data.targetEnvironment = {
                            name: name, cname: inactiveCname, isActive: false
                        }
                    ...
                    data.targetEnvironment.url = ["http://", data.targetEnvironment.cname, ".elasticbeanstalk.com"].join("");
