var Q = require('q'), _ = require('lodash'), fs = require('fs'), l = require('../../../lib/logger.js'), path = require('path'), helpers = require('../../../lib/helpers');

module.exports = function (config, services, args) {

    var s3 = new services.AWS.S3(), eb = new services.AWS.ElasticBeanstalk(), region = config.Region;
    ...
    function createVersion(applicationName, versionLabel, bucket, key) {
        ...
        return Q.ninvoke(eb, "createApplicationVersion", {
            ApplicationName: applicationName, VersionLabel: versionLabel, AutoCreateApplication: true, SourceBundle: {
    ...
    return {
        activate: function (fsm, data) {
            ...
            upload(data.bucket, data.sourceBundleKey, fs.createReadStream(path.join(process.cwd(), args.package))).then(function (result) {
